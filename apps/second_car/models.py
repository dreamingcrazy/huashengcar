...
class CarStyle(models.Model):
    '''车辆的类型：两厢轿车，三厢轿车，跑车，suv,mpv,皮卡，面包车'''
    name = models.CharField(max_length=10,verbose_name='车辆的类型')
    image = models.ImageField(upload_to='type',verbose_name='车型代表性图片')
...
class Brande(models.Model):
    name = models.CharField(max_length=10,verbose_name='车品牌')
...
class CarDetail(models.Model):
    ...
    user = models.ForeignKey('user.UserInfo', verbose_name='所属的用户')
    style = models.ForeignKey('CarStyle', verbose_name='所属的类型')
    brande = models.ForeignKey('Brande', verbose_name='所属的品牌')
    ...
    car_price = models.DecimalField(max_digits=10,decimal_places=3,verbose_name='二手车交易价')
